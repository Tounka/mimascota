...
export const BtnSeguir = ({ boolSeguido, setBoolSeguido, mascotaParaSeguir }) => {
    ...
    console.log(mascotaParaSeguir.mascotaId, mascotaUsuarioSeleccionada.mascotaId)
    const manejarSeguir = async () => {
        ...
        // Verificar que mascotaParaSeguir esté definida y tenga las propiedades necesarias
        const seguidores = (mascotaParaSeguir && mascotaParaSeguir.seguidores) || [];
        const seguidos = (mascotaParaSeguir && mascotaParaSeguir.seguidos) || [];

        try {
            // Verificar si ya sigo a esta mascota
            if (seguidores.includes(miMascotaId)) {
                // Dejar de seguir: eliminar el ID de los arrays de seguidores y seguidos
                const nuevosSeguidores = seguidores.filter(id => id !== miMascotaId);
                const nuevosSeguidos = seguidos.filter(id => id !== mascotaId);
                ...
                await updateDoc(doc(db, "mascotas", mascotaId), {
                    seguidores: [...seguidores, miMascotaId] // Agrega el ID a la lista de seguidores de la mascota
                });

                await updateDoc(doc(db, "mascotas", miMascotaId), {
                    seguidos: [...seguidos, mascotaId] // Agrega el ID a la lista de seguidos de la mascota del usuario
                });
